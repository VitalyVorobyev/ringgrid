...
- [ ] A single documented validation runbook is produced in session artifacts with exact commands and required outputs for:
  - blur=3.0 synth eval (`n=10`)
  - `run_reference_benchmark.sh`
  - `run_distortion_benchmark.sh`
- [ ] `templates/accuracy-report.md` is used to report all three gates with baseline/after-change deltas.
- [ ] Validation handoff template content is standardized for PERF tasks (required metric table fields and artifact paths).
- [ ] At least one dry-run validation report is produced from current PERF baseline/after data showing the standardized format is usable.
- [ ] Backlog/workflow references are aligned so future PERF tasks explicitly point to this standardized suite.
...
- [x] Other: standardize validation orchestration/reporting around existing scripts (`run_synth_eval.py`, `run_reference_benchmark.sh`, `run_distortion_benchmark.sh`)
