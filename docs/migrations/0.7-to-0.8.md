...
> **Archived pre-1.0 upgrade note.** This page is kept for history and is not part
> of the user guide. For the `BoardLayout` removal, see [0.8 → 0.9](0.8-to-0.9.md).

Release 0.8 introduces the compositional target model (hex/rect lattices,
plain/coded markers, origin fiducials), retiers the public API, and renames a
handful of fields. This page lists the breaking changes per interface with
concrete before/after snippets. The pre-1.0 policy favors the cleaner design over
source stability, but most changes are shims or renames with compatibility
aliases, so migration is mechanical.
