...
> **2026-07-04 note:** the [live performance
> dashboard](https://vitalyvorobyev.github.io/ringgrid/performance/) is
> generated by `tools/gen_pages_perf.py` and now covers a **varied** scene set
> — a real capture, a sparse and a dense coded hex board, and a plain rect
> target — each with a rendered **detection overlay**. The static numeric
> snapshots further down this page (distortion / blur eval) predate the 0.8.0
> algorithm changes and have not been re-run for this release; treat the live
> dashboard as the current performance reference.
