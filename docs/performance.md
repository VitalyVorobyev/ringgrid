...
> **2026-07-05 note:** the [live performance
> dashboard](https://vitalyvorobyev.github.io/ringgrid/performance/) is
> generated by `tools/gen_pages_perf.py` and covers a **varied** scene set
> — a real capture, a sparse and a dense coded hex board, and a plain rect
> target — each with a rendered **detection overlay**. It was regenerated for
> 0.10.1 on the radsym 0.4.1 backend (proposal stage ~7–27 % faster; detection
> accuracy unchanged). The static numeric snapshots further down this page
> (distortion / blur eval) predate the 0.8.0 algorithm changes and have not
> been re-run since; treat the live dashboard as the current performance
> reference.
