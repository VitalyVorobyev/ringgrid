...
- `crates/ringgrid/src/ring/radial_estimator.rs`
...
- [x] Shared radial-estimator core is introduced and used by both inner and outer estimators.
- [x] Stage-specific behavior is explicit (policy/gate/hypothesis selection), not encoded as ad-hoc branching inside one giant function.
- [x] Duplication between `inner_estimate` and `outer_estimate` is materially reduced, with clear module boundaries.
- [x] Existing estimator tests pass and new regression tests cover both inner and outer paths through the shared core.
- [x] `cargo test --workspace --all-features` passes.
- [x] `cargo clippy --all-targets --all-features -- -D warnings` clean.
- [x] Validation evidence includes:
